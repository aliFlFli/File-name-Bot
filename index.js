...
function removeSerialByIndex(index) {
  // index مثل همون شماره‌ای هست که توی خروجی /serials نمایش داده میشه (از ۱ شروع میشه)
  const entries = Array.from(serialsMap.entries()); // Map ترتیب درج رو حفظ میکنه
...
bot.command('delserial', async (ctx) => {
  ...
  if (!arg || !Number.isInteger(index) || index < 1) {
    return ctx.reply('❌ شماره سریال رو طبق لیست /serials وارد کن\n\nمثال: /delserial 2');
  }
  
  const removed = removeSerialByIndex(index);
  if (removed) {
    await ctx.reply(`✅ سریال «${removed.english}» (${removed.hashtag}) حذف شد.`);
  } else {
    await ctx.reply(`❌ سریالی با شماره ${index} یافت نشد.\n\nبرای مشاهده لیست فعلی: /serials`);
  }
});
...
bot.on(['document', 'video'], async (ctx) => {
  ...
  const isManualMode = !!(sessionData && sessionData.step === 'upload');
  
  let hashtag;
  let serialEnglishName;
  
  if (isManualMode) {
    // حالت دستی: کاربر از طریق دکمه‌ها اسم سریال رو خودش تایپ کرده
    hashtag = sessionData.hashtag;
    serialEnglishName = sessionData.series;
  } else {
    // حالت خودکار: تشخیص سریال از اسم فایل، بین سریال‌های ثبت‌شده با /addserial
    ...
    hashtag = serial.hashtag;
    serialEnglishName = serial.english;
  }
...
🌐 @KoreaMixPlus • @FaKorea 🌐</b>`;
  ...
  try {
    ...
    // ذخیره در سشن کاربر (اگه در حالت دستی باشیم، همون سشن موجود آپدیت میشه تا step/series حفظ بشه)
    let userSession = getUserSession(userId) || { uploadedFiles: [], fileCount: 0 };
    if (!Array.isArray(userSession.uploadedFiles)) userSession.uploadedFiles = [];
    if (typeof userSession.fileCount !== 'number') userSession.fileCount = 0;
